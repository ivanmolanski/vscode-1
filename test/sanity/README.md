...
Only the targets listed in `targetArtifacts` (`src/context.ts`) can be resolved this way; anything else
fails with an explicit error, so runs must be scoped with `-g`/`-f` to the downloaded artifacts. The
SHA-256 check is skipped in this mode because there is no update service metadata to compare against.
